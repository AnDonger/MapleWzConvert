...
class WzCanvasProperty(WzSubProperty):
    ...
    def value(self) -> Tuple[int, int, int]:
        return (self.width, self.height, self.format + (self.format2 << 8))
...
def _parse_extended(
    ...
    if ext_type in ("Property", "Shape2D#Convex2D"):
        if ext_type == "Property":
            ...
            for child in parse_property_list(reader, base_offset, sub, wz_image):
                sub.add(child)
            return sub
        # Convex children are stored as direct extended values:
        #   <extended-type string-block> + <extended body>
        # They do not have the normal property-list name + tag wrapper.
        convex = WzConvexProperty(name, parent)
        count = reader.read_compressed_int()
        for index in range(count):
            child_ext_type = reader.read_string_block(base_offset)
            if child_ext_type == "Shape2D#Vector2D":
                x = reader.read_compressed_int()
                y = reader.read_compressed_int()
                convex.points.append(WzVectorProperty(str(index), x, y, convex))
                continue
            if child_ext_type == "Shape2D#Convex2D":
                nested_count = reader.read_compressed_int()
                for nested_index in range(nested_count):
                    nested_ext_type = reader.read_string_block(base_offset)
                    if nested_ext_type != "Shape2D#Vector2D":
                        raise ValueError(
                            f"unsupported Convex2D child '{nested_ext_type}' at 0x{reader.position:X}"
                        )
                    x = reader.read_compressed_int()
                    y = reader.read_compressed_int()
                    convex.points.append(WzVectorProperty(f"{index}/{nested_index}", x, y, convex))
                continue
            raise ValueError(
                f"unsupported Convex2D child '{child_ext_type}' at 0x{reader.position:X}"
            )
        return convex
    ...
    if ext_type == "Canvas":
        ...
        canvas.format2 = reader.read_compressed_int()
        reader.skip(4)  # reserved
        length_search_start = reader.position
        payload_offset = None
        payload_length = 0
        # Some legacy Map.wz canvases have extra bytes before the declared
        # length field. Find the length slot that agrees with the enclosing
        # extended-block boundary instead of treating those bytes as pixels.
        for extra in range(0, 17):
            length_pos = length_search_start + extra
            if length_pos + 5 > end_pos:
                break
            reader.seek(length_pos)
            declared_length = reader.read_i32()
            reader.skip(1)  # filler byte before PNG payload
            candidate_payload = reader.position
            candidate_length = end_pos - candidate_payload
            if declared_length > 0 and declared_length - 1 == candidate_length:
                payload_offset = candidate_payload
                payload_length = candidate_length
                break

        if payload_offset is None:
            reader.seek(length_search_start)
            reader.read_i32()  # declared length field
            reader.skip(1)  # filler byte before PNG payload
            payload_offset = reader.position
            payload_length = max(0, end_pos - payload_offset)

        canvas._png_offset = payload_offset
        canvas._png_length = max(0, payload_length)
        reader.seek(end_pos)
        return canvas
