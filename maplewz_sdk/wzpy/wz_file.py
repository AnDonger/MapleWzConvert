...
class WzFile:
    ...
    def _parse_directory(self, directory: WzDirectory) -> None:
        ...
        # Pass 1: collect entries (name, kind, size, checksum, offset). The entries don't
        # store nested data inline — they all point elsewhere in the file.
        entries: List[Tuple[str, int, int, int, int, int, Optional[int]]] = []
        for _ in range(count):
            kind = r.read_byte()
            entry_kind = kind
            entry_string_offset: Optional[int] = None
            name = ""
            if kind == 1:
            ...
            if kind == 2:
                string_offset = r.read_i32()
                entry_string_offset = string_offset
                name = r.read_string_at(self.header.fstart + 1 + string_offset)
                kind = 4
            elif kind in (3, 4):
                name = r.read_string()
            else:
            ...
            entries.append((name, kind, size, checksum, offset, entry_kind, entry_string_offset))

        # Pass 2: recurse / register
        for name, kind, size, checksum, offset, entry_kind, entry_string_offset in entries:
            if kind == 3:
                sub = WzDirectory(name, parent=directory)
                sub._entry_size = size
                sub._checksum = checksum
                sub._entry_kind = entry_kind
                sub._entry_string_offset = entry_string_offset
                ...
                img._checksum = checksum
                img._entry_kind = entry_kind
                img._entry_string_offset = entry_string_offset
