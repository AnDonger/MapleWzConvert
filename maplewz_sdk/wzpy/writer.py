...
def encode_ascii_payload(plaintext: str, combined: bytes) -> bytes:
    ...
    return encode_ascii_payload_bytes(raw, combined)


def encode_ascii_payload_bytes(raw: bytes, combined: bytes) -> bytes:
    """Encrypt bytes using WZ's negative-length ASCII/ANSI string branch."""
    n = len(raw)
    if n > len(combined):
        raise ValueError(
...
def encode_unicode_payload(plaintext: str, combined: bytes) -> bytes:
    ...
    return encode_unicode_payload_bytes(raw, combined)


def encode_unicode_payload_bytes(raw: bytes, combined: bytes) -> bytes:
    """Encrypt bytes using WZ's positive-length Unicode string branch."""
    n = len(raw)
    if n > len(combined):
        raise ValueError(
            f"keystream table is {len(combined)} bytes, need {n}"
...
#                       If sign is -128 for ASCII or +127 for Unicode, an i32
#                       length follows.
#   [length i32]        only when the extension sentinel is used
...
def encode_ansi_string(reader: Any, s: str, encoding: str) -> bytes:
    """Encode a string through WZ's negative-length branch with an ANSI codepage."""
    if s == "":
        return bytes([0])
    raw = s.encode(encoding)
    return encode_ansi_string_bytes(reader, raw)


def encode_ansi_string_bytes(reader: Any, raw: bytes) -> bytes:
    """Encode raw bytes through WZ's negative-length ASCII/ANSI branch."""
    if not raw:
        return bytes([0])
    table = reader._ensure_ascii_combined(len(raw))
    cipher = encode_ascii_payload_bytes(raw, table)
    n = len(cipher)
    if n < 128:
        return _struct.pack("<b", -n) + cipher
    return _struct.pack("<bi", -128, n) + cipher
...
def encode_unicode_string_bytes(reader: Any, raw: bytes) -> bytes:
    """Encode raw UTF-16-like bytes through WZ's positive-length branch."""
    if not raw:
        return bytes([0])
    if len(raw) % 2:
        raise ValueError("unicode string payload byte length must be even")
    char_count = len(raw) // 2
    table = reader._ensure_unicode_combined(char_count)
    cipher = encode_unicode_payload_bytes(raw, table)
    if char_count < 127:
        return _struct.pack("<b", char_count) + cipher
    return _struct.pack("<bi", 127, char_count) + cipher
...
def encode_string_block_with_encoding(reader: Any, s: str, encoding: str) -> bytes:
    """Encode a string property value with an explicit on-wire text encoding."""
    if encoding == "auto":
        return encode_string_block(reader, s)
    if encoding == "unicode":
        return bytes([0x00]) + encode_string(reader, s, prefer_ascii=False)
    if encoding in {"gbk", "gb18030", "big5"}:
        return bytes([0x00]) + encode_ansi_string(reader, s, encoding)
    if encoding == "utf8":
        return bytes([0x00]) + encode_ansi_string_bytes(reader, s.encode("utf-8"))
    if encoding == "utf16le-ansi":
        return bytes([0x00]) + encode_ansi_string_bytes(reader, s.encode("utf-16-le"))
    if encoding == "unicode-be":
        return bytes([0x00]) + encode_unicode_string_bytes(reader, s.encode("utf-16-be"))
    raise ValueError(f"unknown string value encoding: {encoding!r}")
...
def _encode_property_body(prop: Any, reader: Any) -> bytes:
    ...
    if isinstance(prop, WzStringProperty):
        encoding = getattr(prop, "_value_encoding", "auto")
        return encode_string_block_with_encoding(reader, str(prop.value), encoding)
    ...
    if isinstance(prop, WzVectorProperty):
        ...
        for v in prop.points:
            # Convex children are direct extended values, not property-list
            # entries, so there is no name string and no tag byte here.
...
def _encode_canvas_extended_block(canvas: Any, reader: Any) -> bytes:
    ...
    format2(int) + reserved(4) + len_field(i32) + filler(1) + payload``.
    """
    ...
    body += encode_compressed_int(int(canvas.format2))
...
def encode_image_body(image: Any, reader: Any) -> bytes:
    ...
    raw_body = getattr(image, "_raw_body", None)
...
def _entry_kind_and_name_bytes(child: Any, default_kind: int, name: str, reader: Any) -> Tuple[int, bytes]:
    entry_kind = getattr(child, "_entry_kind", default_kind)
    entry_string_offset = getattr(child, "_entry_string_offset", None)
    if entry_kind == 2 and entry_string_offset is not None:
        return 2, _struct.pack("<i", int(entry_string_offset))
    return default_kind, encode_string(reader, name)


def _directory_child_size(child: Any, plan: "_Plan") -> int:
    original_size = getattr(child, "_entry_size", 0)
    if original_size:
        return int(original_size)
    return len(plan.subdir_body[id(child)])
...
def _directory_child_checksum(child: Any, kind: int, plan: "_Plan") -> int:
    if kind == 4:
        return _to_signed_i32(sum(plan.image_body[id(child)]))
    return getattr(child, "_checksum", 0)
...
def _serialize_directory_body_skeleton(
    ...
    for name, child, kind in items:
        entry_kind, name_bytes = _entry_kind_and_name_bytes(child, kind, name, reader)
        body += bytes([entry_kind])
        # Directory entries use the bare-string form (no marker byte);
        # only property names inside images get the marker.
        body += name_bytes
        if kind == 3:
            child_size = _directory_child_size(child, plan)
        else:
            child_size = len(plan.image_body[id(child)])
        checksum = _directory_child_checksum(child, kind, plan)
        body += encode_compressed_int(child_size)
        body += encode_compressed_int(checksum)
...
def _patch_directory_offsets(
    ...
    for name, child, kind in items:
        _entry_kind, name_bytes = _entry_kind_and_name_bytes(child, kind, name, reader)
        cursor += 1  # kind byte
        ...
        cursor += len(encode_compressed_int(_directory_child_checksum(child, kind, plan)))
...
def encode_wz_file(
    ...
    *,
    ...
    out = bytearray()
    out += b"PKG1"
    out += _struct.pack("<Q", total_body_size - fstart)
